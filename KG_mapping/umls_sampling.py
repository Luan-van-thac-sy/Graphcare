...
def two_hop_mapping(mapping_dict, umls_cuis, triple_set, save_path):
    ...
    # Filter out None mappings first
    valid_mappings = {k: v for k, v in mapping_dict.items() if v is not None}
    print(f"Valid mappings: {len(valid_mappings)}/{len(mapping_dict)}")

    print("retrieving first hop triples...")
    first_hop_triples = defaultdict(set)
    for key in tqdm(valid_mappings.keys()):
        umls_id = valid_mappings[key]
    ...
    for key in tqdm(first_hop_triples.keys()):
        for triple in first_hop_triples[key]:
            second_hop_nodes[key].add(triple[0]) if triple[0] not in valid_mappings.keys() else None
            second_hop_nodes[key].add(triple[2]) if triple[2] not in valid_mappings.keys() else None
    ...
    for key in tqdm(second_hop_nodes.keys()):
        ...
        if num_samples == 0:  # Skip if no nodes
            continue
        random_five_nodes = random.sample(list(nodes), num_samples)
...
ccscm2umls = load_data('../exp_data/ccscm2umls.pkl')
ccsproc2umls = load_data('../exp_data/ccsproc2umls.pkl')
atc32umls = load_data('../exp_data/atc32umls.pkl')
...
with ThreadPoolExecutor(max_workers=16) as executor:
    datasets = [
        (ccscm2umls, umls_cuis, triple_set, '../graphs/ccscm_umls'),
        (ccsproc2umls, umls_cuis, triple_set, '../graphs/ccsproc_umls'),
        (atc32umls, umls_cuis, triple_set, '../graphs/atc3_umls')
    ]

    results = list(tqdm(executor.map(threaded_two_hop_mapping, datasets), total=len(datasets)))
